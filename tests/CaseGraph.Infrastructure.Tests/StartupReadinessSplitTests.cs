...
public sealed class StartupReadinessSplitTests
{
    [Fact]
    public async Task InitializeAsync_DoesNotCreateMessageSearchIndexUntilFeatureReadinessRuns()
    {
        ...
        var pathProvider = fixture.PathProvider;

        await initializer.InitializeAsync(CancellationToken.None);
        var messageEventId = await SeedMessageWithMissingFtsRowAsync(pathProvider.WorkspaceDbPath);

        var freshInitializer = new WorkspaceDbInitializer(
            fixture.Services.GetRequiredService<IDbContextFactory<WorkspaceDbContext>>(),
            fixture.Services.GetRequiredService<IWorkspacePathProvider>(),
            fixture.Services.GetRequiredService<WorkspaceDbRebuilder>(),
            fixture.Services.GetRequiredService<IWorkspaceWriteGate>(),
            fixture.Services.GetRequiredService<IClock>()
        );
        await freshInitializer.InitializeAsync(CancellationToken.None);
        ...
        var workPerformed = await freshInitializer.EnsureMessageSearchReadyAsync(CancellationToken.None);
        var secondRun = await freshInitializer.EnsureMessageSearchReadyAsync(CancellationToken.None);

        Assert.True(workPerformed);
    ...
    private sealed class ReadinessFixture : IAsyncDisposable
    {
        ...
        public static Task<ReadinessFixture> CreateAsync()
        {
            var workspaceRoot = Path.Combine(
                Path.GetTempPath(),
                "CaseGraph.StartupReadinessSplitTests",
